...
const MAX_READS = 200;
async function getGraphQLCache({ parser, logger, loadConfigOptions, config, }) {
    var _a;
...
class GraphQLCache {
    constructor({ configDir, config, parser, logger, }) {
        this.getGraphQLConfig = () => this._graphQLConfig;
        this.getProjectForFile = (uri) => {
            try {
                return this._graphQLConfig.getProjectForFile(vscode_uri_1.URI.parse(uri).fsPath);
            }
            catch (err) {
        ...
        this.promiseToReadGraphQLFile = async (filePath) => {
            const content = await (0, promises_1.readFile)(vscode_uri_1.URI.parse(filePath).fsPath, 'utf8');
            const asts = [];
            let queries = [];
    ...
    async updateFragmentDefinitionCache(rootDir, filePath, exists) {
        ...
        else if (fileAndContent === null || fileAndContent === void 0 ? void 0 : fileAndContent.queries) {
    ...
    async updateObjectTypeDefinitionCache(rootDir, filePath, exists) {
        ...
        if (!exists) {
            ...
            await this.updateObjectTypeDefinition(rootDir, filePath, fileAndContent.queries);
        }
    }
