...
class MessageProcessor {
    constructor({ logger, fileExtensions, graphqlFileExtensions, loadConfigOptions, config, parser, tmpDir, connection, }) {
        ...
        this._tmpUriBase = vscode_uri_1.URI.file(this._tmpDirBase).toString();
        ...
        if (!(0, node_fs_1.existsSync)(this._tmpDirBase)) {
            void (0, mkdirp_1.default)(this._tmpDirBase);
        }
    }
    get connection() {
    ...
    async handleWatchedFilesChangedNotification(params) {
        ...
        return Promise.all(params.changes.map(async (change) => {
            ...
            if (change.type === graphql_language_service_1.FileChangeTypeKind.Created ||
                ...
                const { uri } = change;
                const text = (0, node_fs_1.readFileSync)(vscode_uri_1.URI.parse(uri).fsPath, 'utf-8');
    ...
    async _cacheSchemaFile(_uri, project) {
        ...
        const isFileUri = (0, node_fs_1.existsSync)(uri);
        let version = 1;
        if (isFileUri) {
            const schemaUri = vscode_uri_1.URI.file(path.join(project.dirpath, uri)).toString();
            const schemaDocument = this._getCachedDocument(schemaUri);
            if (schemaDocument) {
                version = schemaDocument.version++;
            }
            const schemaText = (0, node_fs_1.readFileSync)(uri, 'utf8');
    ...
    _getTmpProjectPath(project, prependWithProtocol = true, appendPath) {
        ...
        if (!(0, node_fs_1.existsSync)(projectTmpPath)) {
            void (0, mkdirp_1.default)(projectTmpPath);
        }
        if (appendPath) {
    ...
    async _cacheSchemaPath(uri, project) {
        try {
            const files = await (0, fast_glob_1.default)(uri);
    ...
    async _cacheConfigSchema(project) {
        try {
            ...
            if (schema) {
                ...
                if (!cachedSchemaDoc) {
                    await (0, promises_1.writeFile)(fsPath, schemaText, 'utf8');
                    await this._cacheSchemaText(uri, schemaText, 1);
                }
                if (cachedSchemaDoc) {
                    (0, node_fs_1.writeFileSync)(fsPath, schemaText, 'utf8');
